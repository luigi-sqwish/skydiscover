...
3. Build ALMA's `alfworld` Docker image from the ALMA repo.
...
```bash
uv run skydiscover-run benchmarks/alma_alfworld/initial_program.py \
  benchmarks/alma_alfworld/evaluator.py \
  -c benchmarks/alma_alfworld/config_adaevolve.yaml \
  --search adaevolve \
  --iterations 20
```
